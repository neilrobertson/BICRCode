...
use constant kElementCol => 0;
use constant kReferenceCol => 1;
use constant kStartCol => 2;
use constant kEndCol => 3;
use constant kLogRatioCol => 4;
...
my ($basename) = $infile =~ /(.*).id.txt$/;
$basename =~ s/input/output/;

my $oOUT = $basename . "_$alpha" . ".chipotle.STDOUT";
...
my $dat_file = $oOUT . "." . getppid();
tie @windowDataRows, 'Tie::File', $dat_file;

my $numWindows = RunSlidingWindow($infile);
print STDERR "Sliding window done.\n";

# Now perform multiple testing correction
my @windowDataRowsTested;
my $multipleTestingSignificanceThreshold =
  FindMultipleTestingCorrectionThreshold(\@pValueCollection, $alpha, $method);
...
untie(@windowDataRows);
unlink($dat_file);

# Reads in intermediate features file and collapses significant,
# overlapping windows into peaks.  Returns array containing count of
...
sub CollapseWindowsIntoFeatures {
  ...
  my $out = $basename . "_$alpha" . "_chipotle_output.txt";
  ...
  foreach my $window (@windowDataRowsTested) {
    ...
    if ($cols[kOutSigCallCol] == 1) {
      ...
      if ($collectingFeature == 1) {
        ...
        my $newRight = int(($cols[kOutEndCol] + $cols[kOutStartCol]) / 2);
        my $distChanged = $newRight - $featureRight;
        ...
        if ($flagGAP) {
          ...
          my @temp = ($featureID, $featureRefseq, $featureLeft,
                      $featureRight, $value, $pvalue, $featureMaxWin);
      ...
      if ($collectingFeature == 1) {
        # If the previous data row was in a feature and this row is
        # not signficant, then we're not collecting windows into that
        # feature output that feature
        my $value = sprintf("%.3f", $featureValStat->mean());
        my $pvalue = sprintf("%.3e", $featurePvalStat->mean());
        if ($featureLeft == $featureRight) {
          my $extend = int($stepSize / 2);
...
# This subroutine scans through the list of windows and compares their
# p-value to the multiple-testing-correction derived significance
# threshold. It returns the integer number of significanct features.
...
sub EvaluateSlidingWindowResults {
  ...
  foreach my $wref (@windowDataRows) {
    # foreach my $wref (@$tiedWindowDataRows) {
...
# The base code ported over from the ChIPotle algorithm. Evaluates a
# sliding window of genomic tiling ChIP data against a Gaussian error
# function to find significant peaks. Returns the integer ID of the
# last window examined, which is a proxy for the number of windows
# examined.
...
sub RunSlidingWindow {
  ...
  while ($currentRow <= $finalRow) {
    ...
    if (($featurePos >= $windowLeft)
        and ($featurePos <= $windowRight)
        and ($cols[kReferenceCol] eq $windowCurrentReferenceSeq)){
    ...
    if ($currentState == kWithinWindow) {
      # Cache current reference sequence. Not actually needed, but
      # formally we should do this while inside this loop
      $windowCurrentReferenceSeq = $cols[kReferenceCol];
      ...
      # Only compute window values if: 1. We've run downstream of the
      # current window 2. There are one or more features in the array
      # of values. The 2nd condition accounts for cases where a window
      # cannot be filled. Primarily, this will be due to gaps in the
      # genomic tiling array coverage.
      if (($currentState == kPastWindow) and ($windowCountFeatures > 0)) {
        ...
        my $windowPvalue = 1;
        $windowPvalue = ErrorFunction($windowAverageLogRatio /
                                      ($backgroundStdev / sqrt($windowCountFeatures))) / 2;
        # Output to temporary file
        my @temp = ($windowID, $cols[kReferenceCol], $windowLeft, $windowRight,
                    $windowAverageLogRatio, $windowPvalue);
        # Store references to the window data

        my $foo = join("@", @temp);

        # push(@windowDataRows, \@temp);
        push(@windowDataRows, $foo);
      ...
      # Slide the window N base pairs from start of previous one if
      # and only if we have entered this loop due to passing the end
      # of the previous window
      if ($currentState == kPastWindow) {
        # Set up the sliding window's new coordinates
        $windowLeft = $windowLeft + $stepSize;
        # Backup until feature is within new window. This is not
        # robust for dealing with array gaps or changes in reference
        # sequence
        if ($featurePos > $windowLeft) {
...
# This routine calculates the standard deviation of the
# negative-valued (unbound) portion of the ChIP array. In the original
# implementation, the entire array is loaded into memory to find the
# background distribution. That's not viable for a high-volume
# implemenation, so instead, I pass once through the entire data set
# and harvest all the background values.
...
sub CalculateBackgroundStdev {
  ...
  my ($datasetMeanValue, $sumDeviation, $countBackgroundElements) =
    _calculate_sd(\@rows);

  print SOUT "Data set mean: $datasetMeanValue\n";
  if (abs(0 - $datasetMeanValue) > 0.5) {
    print SOUT "Warning: this data set's mean value is really skewed away from a mean of zero!\nRe-running chipotle using '--transform $datasetMeanValue'\n";
    $transform = $datasetMeanValue;
    ($datasetMeanValue, $sumDeviation, $countBackgroundElements) =
      _calculate_sd(\@rows);
  }

  my $tempBackgroundStdev =
    sqrt($sumDeviation / ($countBackgroundElements - 1));

  return $tempBackgroundStdev;
}

# FindMultipleTestingCorrectionThreshold(\@array_of_pvalues, 0.05, 'BH')

# This subroutine takes an array of p-values, an alpha, and a MTC
# method name and returns a corrected significance
# threhold. Currently, only the Benjamini and Hochberg (1995) FDR
# method is supported. The subroutine returns the threshold as a
...
# This is a subroutine name from the original ChIPotle source code. It
# returns scalar maximum value for array. It assumes all values in
# array are numeric.
...
# This is a subroutine name from the original ChIPotle source code. It
# returns scalar minimum value for array. It assumes all values in
# array are numeric
sub GetMin {
	my $arrayref = shift or die;
	my @unsorted = @{ $arrayref };
...
sub _calculate_sd {
  ...
  my $countBackgroundElements;
  my ($sumDeviation, $sumAllValues, $countAllElements, $regionOLD) = (0,0,0,'NA');

  foreach my $row (@$rows) {
    my @cols = @$row;
    my $value = $cols[kLogRatioCol];
    # Captures the start (lowest) coordinate for each unique region
    # (assume data is sorted)
    my $region = $cols[kReferenceCol];
    my $start = $cols[kStartCol];

    unless ($regionOLD eq $region){
      $referenceSequenceBegin{$region} = 99999999999999999;
    }
    ...
    if ($value < 0) {
      # Subtracts each value from 0, no estimate of the mean or
      # median
  ...
  my $datasetMeanValue =
    sprintf("%.3f", ($sumAllValues / $countAllElements));

  return ($datasetMeanValue, $sumDeviation, $countBackgroundElements);
}
