...
if __name__ == '__main__':
    ...
    for o, a in opts:
        if o=="--combined":
            ...
            print "Comparison group:",compare
        elif o=="--window":
            windowSize = int(a)
            print "window size:", a
        elif o=="--out":
    ...
    for compare in compareList:
        for i in compare:
            headerString += "\t" + i
        headerString += "\tmean\tstdDev\tp-chi.sq\tCoverageFlag"
        ...
        for compare in compareList:
            ...
            for x in xrange(0,len(windowData),2):
                ...
                for y in xrange(1,len(windowData),2):
                    ...
                    if (windowData[x] + windowData[y]) == 0  and y-x ==1:
                        outputString += "\t" + "NaN" 
                        coverageFlag = False                 
                             
                    elif ((windowData[x] + windowData[y]) == 0 or (windowData[x] + windowData[y]) < minimumCoverage)  and y-x ==1:
                    ...
                    elif  y-x == 1 and (windowData[x] + windowData[y]) > 0:
                        windowPercent.append((float(windowData[x])/float(windowData[x]+windowData[y])) * 100.0)
                        outputString += "\t" + str(windowPercent[-1])
                        methList.append(windowData[x])
                        unmethList.append(windowData[y])
            ...
            if coverageFlag:
                ...
                try:
                    ...
                    p = 1.0
                outputString += "\t" + str(numpy.mean(arr,axis=None)) + "\t" + str(numpy.std(arr, axis=0)) + "\t" + str(p) + "\t" + str(int(coverageFlag))
            else:
                outputString += "\t" + "NaN" + "\t" + "NaN" + "\t" + "NaN" + "\t" + str(int(coverageFlag))
